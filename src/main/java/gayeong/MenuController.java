...
public class MenuController extends HttpServlet {
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response)
		...
		if (action == null || action.equals("/manager2.do")) {
			 ...
			 * 
			 * // 로그인 정보 불러오기 HttpSession session = request.getSession(); PrintWriter out
			 * =response.getWriter();
			 * 
			 * String user_id = (String) session.getAttribute("id");
			 * System.out.println(user_id); if(user_id != null && user_id.length() != 0) {
			 * 
			 * out.print("장바구니에 메뉴가 담김");
			 * 
			 * String menu_id = request.getParameter("menu_id");
			 * System.out.println(menu_id); List<MenuVO> cart_list = (List<MenuVO>)
			 * menuDAO.cart_list(menu_id,user_id);
			 * 
			 * request.setAttribute("cart_list", cart_list);
			 * 
			 * request.setAttribute("msg", "login"); nextpage =
			 * "/gayeong/store_page/cart.jsp";
			 * 
			 * } else { // id 값이 없으면 로그인 페이지로 이동
			 * 
			 * out.print("로그인 해주셈");
			 * 
			 * request.setAttribute("msg", "null_id"); nextpage =
			 * "/gayeong/store_page/infoPage.jsp";
			 * 
			 * session.invalidate(); }
			 * 
			 * 
